...
class NaughtsAndCrossesState():                            # 连接到treeNode的state中
    def __init__(self,b,tar,obstacles):
        ...
        self.lane1 = numpy.zeros(shape=(10, 2))
        self.lane2 = numpy.zeros(shape=(10, 2))
        self.lane3 = numpy.zeros(shape=(10, 2))
        self.lane4 = numpy.zeros(shape=(10, 2))
    ...
    def takeAction(self, action):                        # node.state.takeAction(action)，这一步只更新棋面和棋手
        ...
        newState.reward = self.reward - 10 - 100 * (newState.lastlane[0]-self.target[0])*(newState.lastlane[0]-self.target[0]) - 0.5 * (400-newState.laststate[0])
        if action.act == 1:
