...
class SampleBasis:
    ...
    def __init__(self, traj_point, theta_thr, from_decision, s_init):
        # v_tgt = from_decision.v_end
        ...
        self.dist_samp = list(np.linspace(from_decision.s_end- s_init - 0.2,from_decision.s_end - s_init + 0.2, 20)) #calibration
        self.dist_prvw = self.dist_samp[0]
        # self.d_end_samp = [0]
        self.d_end_samp = list(np.linspace(from_decision.d_end - 0.1 , from_decision.d_end + 0.1, 20))
        self.v_end = from_decision.v_end      # for cruising
        self.acc_end = from_decision.acc_end
...
class LocalPlanner:
    ...
    def __JudgeStatus(self,traj_point, path_points, obstacles, samp_basis, from_decision):
        ...
        if from_decision.act in [1,2]:
            self.status = "planning_back"
        if from_decision.act in [3,4,5]:
            self.status = "following_path"
        if from_decision.act == 6:
            self.status = "planning_out"
