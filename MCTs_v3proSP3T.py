...
class NaughtsAndCrossesState():  # 连接到treeNode的state中
    ...
    def takeAction(self, action):  # node.state.takeAction(action)，这一步只更新棋面和棋手
        ...
        newState.reward = self.reward - 10 - 1000 * (newState.lastlane[0] - self.target[0]) * (
                    newState.lastlane[0] - self.target[0]) - 0.5 * (self.target[1] + 150 - newState.laststate[0])
        if action.act == 1:
        ...
        if action.act == 3:
            newState.reward = newState.reward + 1000
        if action.act == 5:
            newState.reward = newState.reward - 50 * (self.target[1] + 150 - newState.laststate[0])
        if action.act == 6:
...
if __name__ == "__main__":
    ...
    map = [3, 1, 1549.8999999996904, 44.333333333333336]
    # obstacles = [[0, 100, 25], [0, 150, 20], [1, 180, 10], [1, 120, 20], [2, 80, 15], [3, 80, 10], [4, 80, 15]]
    obstacles =  [[0.00000000e+00, 1.22230000e+03, 3.84660840e+01],
 [0.00000000e+00, 1.99800000e+02, 2.65687692e+01]]
    ...
    # end = time.time()
    # print(end - start)
    # [1.00000000e+00, 1.39900000e+02, 3.34867287e+01],
    # [1.00000000e+00, 7.78000000e+01, 3.68350461e+01],
    # [0.00000000e+00, 2.69000000e+01, 8.33863084e+00],
    # [1.00000000e+00, 8.90000000e+00, 3.64456328e+00],
    ...
    # [1.00000000e+00, 9.63100000e+02, 3.93480878e+01],
    # [1.00000000e+00, 9.81300000e+02, 3.75637993e+01],
    # [1.00000000e+00, 9.14400000e+02, 3.95314354e+01],
